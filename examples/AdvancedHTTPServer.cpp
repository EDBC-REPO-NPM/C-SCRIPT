...
void server( int process ){
    ...
    auto server = http::server([=]( http_t cli ){ 
        ...
        console::log("${0} ${1}",cli.path,process);

        if( fs::exists_file(dir)<0 ){
            cli.write_header( 404,{{ "content-type", "text/plain" }} );
            cli.end( format("404: Oops time: ${0}",date::fulltime()) ); return;
        }
        ...
        if( cli.headers["Range"].empty() ){
            ...
            uint rang[2]; rang[0] = std::stoi( range[0] );
                 rang[1] = std::min( rang[0]+(1024*1024*10), str.size()-1 );

            cli.write_header( 206, {
                { "Content-Range", format("bytes ${0}-${1}/${2}",rang[0],rang[1],str.size()) },
                { "Content-Type",  path::mimetype(dir) }, 
                { "Accept-Range", "bytes" }
            });
