...
namespace HTTP_NODEPP {
    ...
    string_t _get_http_status( int status ){
        switch( status ){
            ...
            default: throw err_t(format("Status ${0} Not Found",status)); break;
        }
    }
        ...
        int read_header(){
            try {
                ...
                do { string_t line = readline();
                    ptr_t<uint> index =_a.search( line );
                ...
                if( !regex::test( init[1], "^\\d+" ) ) {
                    ...
                    url    = format( "http://${0}${1}${2}", headers["Host"], path, search );
                } else {
                    version= init[0]; status = stoi(init[1]);
                }          return 1;
            } catch(...) { return 0; }
        }

        void write_header( string_t method, string_t path, string_t version, header_t headers ){ 
            string_t res; res += format("${0} ${1} ${2}\r\n",method,path,version);
            for( auto x:headers ){ res +=format("${0}: ${1}\r\n",x.first,x.second); }
                                   res += "\r\n"; write( res );
        }
