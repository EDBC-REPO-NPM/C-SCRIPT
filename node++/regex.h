...
class regex_t { 
    ...
    REGEX_NODEPP::NODE* compile( string_t* _reg ){ 
        ...
        for( uint i=0; i<_reg->size(); i++ ){ char x = (*_reg)[i];
            ...
            if( x == ']' || x == '}' || x == ')' ){
                Serial.write(("Error regex at character: "+string::to_string(i)).c_str());
            }
            
            if( x == '?' || x == '*' || x == '+' ){ 
                if( prv == nullptr ){ Serial.write(("Error regex at character: "+string::to_string(i)).c_str()); }
            ...
            if( x == '{' ){ i++; string_t s; int j=0, k=0; REGEX_NODEPP::NODE* nw = new REGEX_NODEPP::NODE;
                ...
                while( i<_reg->size() ){ int y = (*_reg)[i];
                    ...
                    if( !isdigit(y) ){ Serial.write(("Error regex at character: "+string::to_string(i)).c_str()); } s.push(y); i++;
                }   if( k!=-1 ){ Serial.write(("Error regex at character: "+string::to_string(i)).c_str()); }
            ...
            if( x == '[' ){ i++; string_t s; int j=0, k=0, n=0; add_new( act, prv, 0 );
                ...
                act->alt = new REGEX_NODEPP::NODE; for( uint i=0; i<s.size(); i++ ){ 
                    if( s[i+1] == '-' && (i+2)<s.size() ){
                        int a = min( (int)s[i], (int)s[i+2] ); 
                        int b = max( (int)s[i], (int)s[i+2] );
                        for( int j=a; j<=b; j++ ){
            ...
            if( x == '(' ){ i++; string_t s; add_new( act, prv, 0 ); long int j=0;
                ...
                if( j!=-1 ){ Serial.write(("Error regex at character: "+string::to_string(i)).c_str()); }
